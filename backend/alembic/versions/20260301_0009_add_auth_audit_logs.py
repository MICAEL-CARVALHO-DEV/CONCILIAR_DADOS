...
def upgrade() -> None:
    ...
    existing_indexes = {idx["name"] for idx in inspector.get_indexes("auth_audit_logs")}
    if "ix_auth_audit_logs_user_id" not in existing_indexes:
        op.create_index("ix_auth_audit_logs_user_id", "auth_audit_logs", ["user_id"])
    if "ix_auth_audit_logs_event_type" not in existing_indexes:
...
def downgrade() -> None:
    ...
    existing_indexes = {idx["name"] for idx in inspector.get_indexes("auth_audit_logs")}
    if "ix_auth_audit_logs_event_type" in existing_indexes:
        op.drop_index("ix_auth_audit_logs_event_type", table_name="auth_audit_logs")
    if "ix_auth_audit_logs_user_id" in existing_indexes:
