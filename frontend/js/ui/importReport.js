(function (global) {
  ...
  function buildMetricGridHtml(items, escapeHtmlFn) {
    var escapeHtml = typeof escapeHtmlFn === "function" ? escapeHtmlFn : escape;
    var safeItems = safeArr(items);
    if (!safeItems.length) return '<p class="muted small">Sem metricas disponiveis.</p>';

    return '<div class="import-metric-grid">' + safeItems.map(function (item) {
      var label = item && item.label != null ? escapeHtml(String(item.label)) : "-";
      var value = item && item.value != null ? escapeHtml(String(item.value)) : "-";
      return ''
        + '<article class="import-metric-card">'
  ...
  function buildImportSummaryHtml(report, stateRecords, lastImportedPlanilha1Aoa, escapeHtmlFn, fmtDateTimeFn, buildPlanilha1AoaFn, normalizeLooseTextFn, buildPlanilha1HtmlFn) {
    ...
    return "" +
      ...
      buildMetricGridHtml([
        { label: "Linhas lidas", value: report.totalRows || 0 },
        { label: "Linhas validas", value: report.consideredRows || 0 },
        { label: "Linhas ignoradas", value: report.skippedRows || 0 },
        { label: "Novos registros", value: report.created || 0 },
        { label: "Registros atualizados", value: report.updated || 0 },
        { label: "Sem alteracao", value: report.unchanged || 0 },
        { label: "Duplicidade por ID", value: report.duplicateById || 0 },
